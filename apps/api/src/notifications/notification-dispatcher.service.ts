...
export class NotificationDispatcherService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    await this.eventBus.consume(CONSUMER_GROUP, (event) =>
  ...
  private async handleEvent(event: DomainEventMessage): Promise<void> {
    ...
    await withTenantContext(
      ...
      async (queryRunner) => {
        ...
        if (!notification) {
          this.logger.warn(
            `notification ${notificationId} not found (already processed or invalid)`,
          );
          return;
        }
        ...
        if (!channelImpl) {
          await this.markFailed(
            queryRunner,
            notificationId,
            `no channel implementation registered for "${notification.channel}"`,
          );
          return;
        }
