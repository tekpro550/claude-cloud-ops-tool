...
async function waitForStatus(migrator: Client, id: string, timeoutMs = 10000) {
  ...
  while (Date.now() < deadline) {
    const { rows } = await migrator.query(`SELECT status, sent_at FROM notifications WHERE id = $1`, [id]);
...
async function main() {
  ...
  try {
    ...
    assert(emailResult.status === "sent", `email notification dispatched successfully (status=${emailResult.status})`);
    assert(emailResult.sent_at !== null, "sent_at timestamp was recorded for the email notification");
    ...
    const whatsappResult = await waitForStatus(migrator, whatsappNotification.id);
    assert(
