import {
  Body,
  Controller,
  Delete,
  Get,
  HttpCode,
  Param,
  ParseUUIDPipe,
  Patch,
  Post,
  UseGuards,
} from '@nestjs/common';
import { CurrentTenantId } from '../platform/http/current-tenant.decorator';
import { TenantHeaderGuard } from '../platform/http/tenant-header.guard';
import { CreateResourceDto, UpdateResourceDto } from './resources.dto';
import { ResourcesService } from './resources.service';

@UseGuards(TenantHeaderGuard)
@Controller('resources')
export class ResourcesController {
  constructor(private readonly resources: ResourcesService) {}

  @Get()
  list(@CurrentTenantId() tenantId: string) {
    return this.resources.list(tenantId);
  }

  @Get(':id')
  get(@CurrentTenantId() tenantId: string, @Param('id', ParseUUIDPipe) id: string) {
    return this.resources.get(tenantId, id);
  }

  @Get(':id/dashboard')
  dashboard(
    @CurrentTenantId() tenantId: string,
    @Param('id', ParseUUIDPipe) id: string,
  ) {
    return this.resources.dashboard(tenantId, id);
  }

  @Post()
  create(@CurrentTenantId() tenantId: string, @Body() dto: CreateResourceDto) {
    return this.resources.create(tenantId, dto);
  }

  @Patch(':id')
  update(
    @CurrentTenantId() tenantId: string,
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: UpdateResourceDto,
  ) {
    return this.resources.update(tenantId, id, dto);
  }

  @Delete(':id')
  @HttpCode(204)
  remove(@CurrentTenantId() tenantId: string, @Param('id', ParseUUIDPipe) id: string) {
    return this.resources.remove(tenantId, id);
  }
}
