...
async function main() {
  ...
  try {
    // ---- Groups ----
    const group = await groups.create(tenant.id, {
      name: 'Cloud Support',
      description: 'Azure/AWS tickets',
    });
    ...
    const groupUpdated = await groups.update(tenant.id, group.id, {
      description: 'Updated description',
    });
    assert(
      groupUpdated.description === 'Updated description',
      'group description updated',
    );
    assert(
      groupUpdated.name === 'Cloud Support',
      'unspecified fields untouched on group update',
    );

    let groupNotFound: any = null;
    try {
      await groups.update(tenant.id, '00000000-0000-4000-8000-000000000000', {
        name: 'x',
      });
    } catch (err) {
      groupNotFound = err;
    ...
    assert(
      slaPolicy.first_response_target_minutes === 60,
      'SLA policy created with correct target minutes',
    );
    const slaUpdated = await slaPolicies.update(tenant.id, slaPolicy.id, {
      resolutionTargetMinutes: 600,
    });
    assert(
      slaUpdated.resolution_target_minutes === 600,
      'SLA policy resolution target updated',
    );
    ...
    assert(
      ticketType.default_group_id === group.id,
      'ticket type created with defaultGroupId resolved',
    );
    assert(
      ticketType.default_sla_policy_id === slaPolicy.id,
      'ticket type created with defaultSlaPolicyId resolved',
    );

    let badGroupRef: any = null;
    try {
      await ticketTypes.create(tenant.id, {
        name: 'Bad type',
        defaultGroupId: '00000000-0000-4000-8000-000000000000',
      });
    } catch (err) {
      badGroupRef = err;
    }
    assert(
      ...
      'creating a ticket type with a nonexistent defaultGroupId is rejected (400)',
    );

    // ---- Agents ----
    const agent = await agents.create(tenant.id, {
      name: 'New Agent',
      email: 'new-agent@example.com',
      groupIds: [group.id],
    });
    assert(
      agent.name === 'New Agent' && agent.email === 'new-agent@example.com',
      'agent created (user + agent row)',
    );
    assert(agent.is_active === true, 'a new agent starts active');
    const agentList = await agents.list(tenant.id);
    assert(
      agentList.length === 1,
      'agent appears in list() joined with user name/email',
    );

    let duplicateEmail: any = null;
    try {
      await agents.create(tenant.id, {
        name: 'Duplicate',
        email: 'new-agent@example.com',
      });
    } catch (err) {
    ...
    assert(
      duplicateEmail?.status === 400,
      'creating an agent with an already-used email is rejected (400)',
    );

    const agentDeactivated = await agents.update(tenant.id, agent.id, {
      isActive: false,
    });
    ...
    assert(
      agentListAfterDeactivate.find((a: any) => a.id === agent.id)
        ?.is_active === false,
    ...
    assert(
      groupDeleteBlocked?.status === 400,
      'deleting a group still referenced by a ticket is rejected (400), not a raw DB error',
    );

    await migrator.query(`DELETE FROM tickets WHERE id = $1`, [
      ticketRows[0].id,
    ]);
    await groups.remove(tenant.id, group.id);
    const groupListAfterDelete = await groups.list(tenant.id);
    assert(
      groupListAfterDelete.length === 0,
      'group deletion succeeds once nothing references it',
    );
